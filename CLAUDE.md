...
UI calls: the lists have filters but no paging or sortable headers (the amendment names neither; the global list sorts soonest-expiry first, which is the reason to open it); in-modal section headings are `.toolbar` + `.t-title-sm`, since `.panel-header` belongs to a panel; plans are checkbox rows in `.rule-child-row` because the design system ships no checkbox class of its own; and the termination reason is a `rows="1"` textarea — `.field` is a fixed 32-px row, so a taller textarea spills out of its box. A multi-row field needs a design-system variant.
