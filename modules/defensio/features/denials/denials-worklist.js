// Defensio denial worklist at #/defensio/denials — every denial on file,
// untriaged first then by amount, with the rail's five slices, the search
// and the filters (separation and tier beside the ones the register always
// had), bulk triage over a same-reason selection and assign on each row.
// Reads go through data/repositories/denials.js and nowhere else.
//
// This feature also owns the deeper link and hands the mount over on it:
// anything after /denials is a denial id (with a tab id after that).
...
export async function render(mount, ctx) {
  ...
  fields.separation.innerHTML = `<option value="">Any separation</option>${opts.separations.map((s) => `<option value="${esc(s.key)}">${esc(s.label)}</option>`).join('')}`;
  fields.tier.innerHTML = `<option value="">Any tier</option>${opts.tiers.map((t) => `<option value="${esc(t)}">${esc(t)}</option>`).join('')}`;
  ...
  mount.addEventListener('click', async (e) => {
    ...
    return ctx.navigate(`/defensio/denials/${tr.dataset.id}`);
  });
  ...
  mount.addEventListener('keydown', (e) => {
    ...
    if (tr && e.target === tr && (e.key === 'Enter' || e.key === ' ')) {
      e.preventDefault();
      ctx.navigate(`/defensio/denials/${tr.dataset.id}`);
    }
  });

  // A triage here, a remittance posted in Claima or a hand-off marked
