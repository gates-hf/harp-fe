...
// reason a re-route needs, a manual close, the answer on a contract hand-off
// and the payer's answer to a reconsideration. Each validates, writes through
// data/repositories/denials.js and says what it did; the screens redraw on
// the commit.
...
export const tierOptions = (selected = '') => denials.TIERS.map((t) => `<option value="${t}"${t === selected ? ' selected' : ''}>${t}</option>`).join('');

export const categoryOptions = (selected = '') => denials.CATEGORIES.map((c) => `<option value="${c}"${c === selected ? ' selected' : ''}>${c}</option>`).join('');

/** The people a denial can be assigned to — the demo's roles, with nobody first. */
export const assigneeOptions = (selected = '') => `<option value="">Unassigned</option>${
  ROLES.map((r) => `<option value="${esc(r.name)}"${r.name === selected ? ' selected' : ''}>${esc(r.name)} — ${esc(r.title)}</option>`).join('')}`;
  ...
  const dialog = modal.open({
    ...
    body: `
      <label class="field">
        ...
        <select id="da-who" aria-label="Assignee">${assigneeOptions(rows.length === 1 ? rows[0].assignee || me : me)}</select>
      </label>`,
    foot: `
...
/**
 * Bulk triage: one category, one tier, one class, one root cause and
 * optionally one route over a selection that shares a payer reason — a
 * triage is a reading of the reason, so a mixed selection is refused with
 * the codes it spans. Separation is not asked here: a contractual adjustment
 * or a TPA fee is reconciled one at a time, from the page.
 */
export async function openBulkTriageDialog(ids = []) {
  ...
  const dialog = modal.open({
    ...
    size: 'md',
    body: `
      <p class="modal__lede">One reading, written on each as a true denial: ${rows.map((d) => `<span class="t-mono-sm">${esc(d.id)}</span>`).join(', ')}.</p>
      <div class="toolbar">
        <label class="field"><span class="icon icon--sm">psychology</span><select id="bt-category" aria-label="Category">${categoryOptions(s.category)}</select></label>
        <label class="field"><span class="icon icon--sm">layers</span><select id="bt-tier" aria-label="Tier">${tierOptions(s.tier)}</select></label>
  ...
  $('#bt-cause').addEventListener('change', () => {
    ...
    if (rc) {
      $('#bt-class').value = rc.suggestedClass;
      $('#bt-route').value = rc.suggestedRoute;
      if (rc.tier) $('#bt-tier').value = rc.tier;
      if (rc.category) $('#bt-category').value = rc.category;
    }
  });
  $('#bt-tier').addEventListener('change', () => { $('#bt-class').value = denials.TIER_CLASS[$('#bt-tier').value] || $('#bt-class').value; });
  $('#bt-save').addEventListener('click', () => {
    const fields = {
      class: $('#bt-class').value, rootCauseId: $('#bt-cause').value, note: $('#bt-note').value.trim(), route: $('#bt-route').value,
      category: $('#bt-category').value, tier: $('#bt-tier').value,
    };
    const problems = [];
    ...
    if (refused.length) console.info('[defensio] bulk triage', refused);
...
export async function askRerouteReason(denial, kind) {
  const dialog = modal.open({
    ...
    size: 'sm',
    body: `
      <p class="modal__lede">The ${esc(denials.routeLabel(denial.route.kind).toLowerCase())} route stays in the history with its work item; the new route creates its own.</p>
      <label class="field field--area"><span class="icon icon--sm">notes</span>
...
export async function openManualResolveDialog(denial) {
  const dialog = modal.open({
    ...
    size: 'sm',
    body: `
      <p class="modal__lede">Every other resolution is derived — a remittance, a hand-off, an appeal, a write-off, a separation. This one is yours, with a reason, and the register says so.</p>
...
/** The answer on the contract hand-off the denial was routed to. */
export async function openHandoffOutcomeDialog(denial) {
  const open = denial.amounts.open;
  const dialog = modal.open({
    title: `Hand-off outcome — ${denial.id}`,
    sub: `${denial.route?.ref || 'Hand-off'} · ${usd(open)} on the contract register`,
    icon: 'handshake',
    size: 'sm',
    body: `
      <p class="modal__lede">The hand-off's answer, recorded here until the underpayment feature works it: won recovers the amount, lost loses it, settled recovers part and carries the rest to a new denial.</p>
      <div class="toolbar">
        <label class="field"><span class="icon icon--sm">flag</span>
...
export async function openPayerAnswerDialog(denial) {
  const dialog = modal.open({
    ...
    size: 'sm',
    body: `
      <p class="modal__lede">Overturned puts the claim back with the payer so its remittance can be posted — that posting is what recovers the denial. Upheld ends this route and hands the denial back, triaged, for an appeal or a write-off.</p>
      <label class="field"><span class="icon icon--sm">flag</span>
