...
/**
 * Depth-first rows: [{ path, depth, row, qty, lineTotal, isBundle }].
...
function walk(id, depth, path, out, seen) {
  cdm.componentRows(id).forEach((part, i) => {
    ...
    out.push({ path: nodePath, depth, row: part.row, qty: part.qty, lineTotal: part.lineTotal, isBundle });
...
export function treeHtml(rootId, { expandAll = false } = {}) {
  ...
  return `
    <table class="tbl">
      <thead>
        <tr><th>Code</th><th>Component</th><th class="num">Qty</th><th class="num">Unit price</th><th class="num">Line total</th></tr>
      </thead>
