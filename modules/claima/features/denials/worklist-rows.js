// The denials worklist's table: the fourteen columns the amendment names,
// one row per denial with a checkbox for the bulk actions and the row's own
// Assign. Markup only — the screen owns the state and the listeners.

import * as denials from '../../../../data/repositories/denials.js';
import { esc } from '../../../../shared/format.js';
import {
  ageHtml, amountHtml, assigneeHtml, classHtml, deadlineHtml, isWithheld, patientHtml, payerName, reasonHtml,
  repeatHtml, rootCauseHtml, routeHtml, scopeHtml, statusHtml, withheldCell,
...
function rowHtml(d, role, state) {
  ...
  return `
    <tr data-id="${esc(d.id)}" tabindex="0" title="Open ${esc(d.id)}">
      ...
      <td><a class="crumb-link t-mono-sm" href="#/claima/denials/${esc(d.id)}">${esc(d.id)}</a>${repeatHtml(d)}</td>
  ...
  return `
    <div class="state-view">
      ...
      <p class="state-view__body">${filtered
        ...
        : 'A denial arrives when a remittance is posted with a line the payer refused. Post one on the Remittances workbench and it lands here untriaged.'}</p>
