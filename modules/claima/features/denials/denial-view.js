// The denial page at #/claima/denials/<id>: the banner and the amounts
// strip, the triage and resolution panels in the bounded pane, and the
// evidence — denied lines, the contract version, the linked records and the
// timeline excerpt — with the history beside it. The two panels own their
// nodes and listeners; the page redraws them whole on every commit.
...
import {
  ageHtml, classHtml, coverLabel, deadlineHtml, payerName, repeatHtml, rootCauseHtml, scopeHtml, statusHtml,
...
export async function render(mount, ctx) {
  ...
  function draw() {
    ...
    ctx.setCrumb([
      { label: 'Claima', path: '/claima/denials' },
      { label: 'Denials', path: '/claima/denials' },
      { label: row.id },
    ]);
  ...
  function bannersHtml(row) {
    ...
    if (row.status === 'Deadline Passed') {
      out.push(alert('critical', 'timer_off', `Appeal deadline passed ${date(dl.appealBy)}`,
        `${usd(row.amounts.open)} is still open and nothing has been written off. A route can still be taken — a write-off request, or a hand-off to Defensio for a payer that reconsiders late.`));
    ...
    if (row.status === 'Untriaged') {
      out.push(alert('info', 'rule', 'Untriaged', 'Read the evidence, give the denial a class and a root cause, then take the route the router suggests or override it.'));
    }
    return out.join('');
  }
  ...
  function amountsHtml(row) {
    ...
    return metricRailHtml([
      ...
      { value: usd(a.recovered), label: 'Recovered', tone: a.recovered ? 'success' : '', title: 'Paid back by a remittance, a hand-off or a manual close' },
      { value: usd(a.writtenOff), label: 'Written off', tone: a.writtenOff ? 'critical' : '', title: 'Written off through the write-off tiers' },
      { value: usd(a.lost), label: 'Lost', tone: a.lost ? 'critical' : '', title: 'Given up on — Defensio lost, or closed by hand as not recoverable' },
      { value: usd(a.open), label: 'Open', tone: a.open ? 'warning' : '', sub: a.transferred ? `${usd(a.transferred)} carried forward` : 'denied − recovered − written off − lost',
