...
// one marked (billed, the expected amount stamped at assembly, what was
// denied and the payer's code), the contract version the claim was priced
// under, the authorisations, referral, eligibility check and documents
// linked to the visit, and the last eight events of the claim's timeline.
// Read-only markup; the page draws it into the Evidence tab.
...
import { contractHtml } from '../claims-assembly/claim-chips.js';
import { eventBadge, eventIcon } from '../lifecycle/lifecycle-chips.js';
...
export function evidenceHtml(denial) {
  ...
  return `
    <div class="toolbar">
      ...
      <span class="t-body-sm">Expected is what the contract said the payer owed when the claim was assembled.</span>
    </div>
...
function linesHtml(denial, claim) {
  ...
  return `
    ...
    const deniedHere = mine ? (denial.scope === 'Line' ? denial.amounts.denied : l.remittance?.denied || 0) : l.remittance?.denied || 0;
    return `
      ...
      other ? ` <a class="badge" href="#/claima/denials/${esc(other.id)}" title="Another denial on this line">${esc(other.id)}</a>` : ''}</td>
...
export const repeatChainHtml = (denial) => {
  ...
  return `
    <div class="rule-child-row">
      ...
      ${chain.map((d) => `<a class="btn btn--${d.id === denial.id ? 'tint' : 'secondary'} btn--sm" href="#/claima/denials/${esc(d.id)}" aria-current="${d.id === denial.id ? 'page' : 'false'}"
